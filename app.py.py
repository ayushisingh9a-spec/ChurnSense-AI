...
if uploaded_file is not None:
    ...
    if st.button("🚀 Predict Churn"):
        ...
        # Remove target and likely identifier columns
        if "Churn" in data.columns:
            data.drop(columns=["Churn"], inplace=True)

        id_keywords=["id","customerid","customer_id","custid","account","phone","mobile","number"]
        drop_cols=[]
        for c in data.columns:
            n=c.lower().replace("_","").replace(" ","")
            if any(k in n for k in id_keywords):
                drop_cols.append(c)
        if drop_cols:
        ...
        # Validation
        if data.empty or len(data.columns)!=len(feature_columns):
            st.error("Dataset could not be aligned with the trained model.")
            st.stop()
